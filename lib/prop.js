...
exports.mapProp = (options) => (target, key) => {
    const Type = options.items;
    baseProp(options, Type, target, key, false);
    const name = target.constructor.name;
    const sc = data_1.schema[name][key];
    if (sc) {
        sc.of = sc.type;
        sc.type = mongoose.Schema.Types.Map;
    }
    console.log(sc);
    console.log(sc.type.constructor.name);
    console.log(typeof sc.type);
    console.log(data_1.schema);
};
